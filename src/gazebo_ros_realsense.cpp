...
namespace gazebo
{
  ...
  void GazeboRosRealsense::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    this->ir2_pub_ = this->itnode_->advertiseCamera(rsTopicRoot + cameraParamsMap_[IRED2_CAMERA_NAME].topic_name, 2);
    this->depth_pub_ = this->itnode_->advertiseCamera(rsTopicRoot + cameraParamsMap_[DEPTH_CAMERA_NAME].topic_name, 2);
    if (pointCloud_)
    {
      this->pointcloud_pub_ = this->rosnode_->advertise<sensor_msgs::PointCloud2>(rsTopicRoot + pointCloudTopic_, 2, false);
    }
  }
